...
function pathCardClassName(interactive: boolean) {
  return [
    'sa-card sa-press flex h-full min-h-[220px] w-full flex-col rounded-[10px] border bg-white p-6 text-left shadow-[0px_1px_2px_-1px_rgba(0,0,0,0.06)] outline-none',
    interactive
...
export function OnboardingWelcomeHub({ onStartScratch, onStartFastTrack }: OnboardingWelcomeHubProps) {
  return (
    <div className="mx-auto flex w-full max-w-[880px] flex-col items-center">
      <h1 className="onb-welcome-title text-center text-[32px] font-semibold leading-10 tracking-[-0.6px] text-[#101828]">
        Welcome to Ulo
      </h1>
      ...
      <div className="mt-10 grid w-full gap-4 sm:grid-cols-2">
        <button
          type="button"
          className={`onb-welcome-card ${pathCardClassName(true)}`}
        ...
        <button
          type="button"
          className={`onb-welcome-card relative ${pathCardClassName(true)}`}
          style={{ ['--onb-stagger' as string]: 1 }}
          onClick={onStartFastTrack}
        >
          <span className="sa-pill absolute top-4 right-4 rounded-full bg-[#C4E5C9] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-[#187930]">
            Fastest
          </span>
          <span className="flex size-10 items-center justify-center rounded-full bg-[#f3f4f6]">
