...
function FeatureIcon({ feature, dimmed }: { feature: FeatureItem; dimmed?: boolean }) {
  ...
  if (feature.iconWrap) {
    return (
      <div
        className={`flex size-11 shrink-0 items-center justify-center rounded-2xl bg-[rgba(15,22,35,0.1)] ${iconClass}`}
      >
  ...
  return <img src={feature.icon} alt="" className={`size-[42px] shrink-0 object-contain ${iconClass}`} />
}
  ...
  return (
    ...
    >
      <img
        ...
        src={preview.src}
        alt={preview.alt}
        className="h-auto w-[80%] max-w-full animate-[feature-preview-fade_0.35s_ease-out]"
        width={preview.width}
...
export function FeaturesShowcase() {
  ...
  return (
    <div className="mx-auto mt-10 flex w-full flex-col items-start gap-6 lg:mt-12 lg:flex-row lg:items-start lg:justify-between">
      <nav
        className="flex w-full shrink-0 flex-col gap-8 lg:w-auto lg:max-w-[463px] lg:gap-10"
        ...
        {FEATURES.map((feature) => {
          ...
          return (
            <button
              ...
              className={
                isActive
                  ? 'w-full rounded-2xl border border-[#e5e7eb] bg-white p-7 text-left shadow-[0_4px_8px_rgba(0,0,0,0.04)] transition-shadow hover:shadow-[0_6px_12px_rgba(0,0,0,0.06)] lg:w-auto lg:p-[29px] lg:shadow-none'
                  : 'flex w-full cursor-pointer items-center gap-4 rounded-2xl px-7 py-1 text-left opacity-50 transition-opacity hover:opacity-65 lg:w-auto lg:px-7'
              }
            >
              {isActive ? (
                <div className="flex gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="font-[family-name:var(--font-landing-heading)] text-base font-bold text-[#111827]">
                      {feature.title}
                    </h3>
                    {feature.description ? (
                      <p className="mt-2 text-sm leading-[1.625] text-[#6b7280]">{feature.description}</p>
                    ) : null}
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <h3
                    className={`font-[family-name:var(--font-landing-heading)] text-base font-bold ${feature.inactiveTitleClass ?? 'text-[#858c99]'}`}
                  >
                    {feature.title}
                  </h3>
                </>
