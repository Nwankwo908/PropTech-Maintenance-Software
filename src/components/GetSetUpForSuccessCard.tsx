...
import verifyVendorsIcon from '@/assets/id-card.png'
import uploadCloudIcon from '@/assets/setup-success/upload-cloud.svg'
...
const ITEM_ICONS: Record<SetupSuccessItemId, string> = {
  ...
  property_details: uploadCloudIcon,
  maintenance_prefs: uploadCloudIcon,
  test_request: uploadCloudIcon,
}
...
export function GetSetUpForSuccessCard({ progress, onClose }: GetSetUpForSuccessCardProps) {
  ...
  return (
    <section
      className="sa-enter pointer-events-auto fixed bottom-6 left-6 z-40 w-[calc(100%-3rem)] max-w-[520px] rounded-[16px] border border-[#f1f5f9] bg-white p-6 shadow-[0px_8px_24px_rgba(16,24,40,0.12)] lg:left-[calc(16rem+1.5rem)]"
      ...
      <div className="mt-5 overflow-hidden rounded-[12px] border border-[#e2e8f0]">
        {progress.items.map((item, index) => (
          <Link
            key={item.id}
            to={item.to}
            className={`sa-press flex items-center justify-between py-4 pl-4 pr-5 ${
          ...
          >
            <span className="flex min-w-0 items-center gap-4">
              ...
              <span className="text-[14px] font-semibold leading-normal text-[#1f2937]">
                {item.label}
              </span>
            </span>
            {item.done ? (
