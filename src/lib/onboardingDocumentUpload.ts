...
const DEMO_BUILDING = '109 South Grove Street'
const DEMO_ADDRESS = '109 South Grove Street, East Orange, NJ'

/** Realistic Ulo onboarding mock extraction from processed uploads. */
export function buildOnboardingExtractionReview(
  ...
  const sourceNames = documents.map((doc) => doc.fileName)
  const primarySource = sourceNames[0] ?? 'uploaded-document.pdf'
  const leaseSource = sourceNames.find((name) => /lease/i.test(name)) ?? primarySource
  const inspectionSource =
    sourceNames.find((name) => /inspection|photo|jpg|png/i.test(name)) ?? primarySource
  const financialSource =
    sourceNames.find((name) => /rent|roll|statement|expense|csv|xlsx/i.test(name)) ?? primarySource
  ...
  const properties: OnboardingExtractedProperty[] = [
    {
      ...
      propertyType: 'Two-family home',
      unitCount: 2,
      unitLabels: 'A and B',
      sourceDocumentName: primarySource,
      confidence: 96,
      selected: true,
      needsReview: false,
    },
  ...
  const residents: OnboardingExtractedResident[] = [
    {
      ...
      building: DEMO_BUILDING,
      phone: '(973) 555-0142',
      email: 'saad.iqbal@example.com',
      leaseStart: '2025-08-15',
      leaseEnd: '2026-08-31',
      sourceDocumentName: leaseSource,
      confidence: 92,
      selected: true,
      needsReview: false,
    },
  ...
  const leases: ExtractedLeaseInfo[] = [
    {
      id: 'ext-lease-1',
      residentName: 'Saad Iqbal & Hina Saad',
      unit: 'B',
      building: DEMO_BUILDING,
      leaseStart: 'August 15, 2025',
      leaseEnd: 'August 31, 2026',
      rentAmount: '$2,850/month',
  ...
  const vendors: OnboardingExtractedVendor[] = [
    {
      id: 'ext-vendor-1',
      name: 'Garden State Plumbing',
      category: 'plumbing',
      phone: '(973) 555-0198',
      email: 'dispatch@gardenstateplumbing.example.com',
      sourceDocumentName: sourceNames.find((n) => /vendor|invoice|plumb/i.test(n)) ?? primarySource,
      confidence: 88,
      selected: true,
      needsReview: false,
    },
    {
      ...
      category: 'electrical',
      phone: '(973) 555-0177',
      email: 'service@brightline.example.com',
      sourceDocumentName: sourceNames.find((n) => /elect/i.test(n)) ?? primarySource,
      confidence: 86,
      selected: true,
      needsReview: false,
    },
    {
      ...
      category: 'general',
      phone: '(973) 555-0165',
      email: 'projects@summitgc.example.com',
      sourceDocumentName: sourceNames.find((n) => /contract/i.test(n)) ?? primarySource,
      confidence: 84,
      selected: true,
      needsReview: false,
    },
  ...
  const maintenanceIssues: OnboardingExtractedMaintenanceIssue[] = [
    {
      ...
      priority: 'high',
      sourceDocumentName: inspectionSource,
      confidence: 91,
      selected: true,
      needsReview: false,
      imageTags: ['Water damage'],
    },
    {
      id: 'ext-issue-2',
      unit: 'A',
    ...
    },
    {
      ...
      building: DEMO_BUILDING,
      category: 'appliance',
      description: 'Damaged dishwasher',
      priority: 'normal',
      sourceDocumentName: inspectionSource,
      confidence: 85,
      selected: true,
      needsReview: false,
      imageTags: ['Kitchen appliance'],
    },
    {
      id: 'ext-issue-4',
      unit: 'B',
      building: DEMO_BUILDING,
      category: 'plumbing',
      description: 'Damaged kitchen faucet',
      priority: 'normal',
      sourceDocumentName: inspectionSource,
      confidence: 84,
      selected: true,
    ...
    },
    {
      ...
      building: DEMO_BUILDING,
      category: 'structural',
      description: 'Standing water in crawl space',
      priority: 'high',
      sourceDocumentName: inspectionSource,
      confidence: 79,
      selected: true,
    ...
    },
    {
      ...
      building: DEMO_BUILDING,
      category: 'structural',
      description: 'Loose deck railing',
      priority: 'high',
      sourceDocumentName: inspectionSource,
      confidence: 82,
      selected: true,
  ...
  const financialRecords: ExtractedFinancialRecord[] = [
    {
      id: 'ext-fin-1',
      recordType: 'Rent roll',
      description: 'Monthly rent schedule — Units A & B',
      amount: '$5,400/mo total',
      period: 'Aug 2025 – Aug 2026',
      sourceDocumentName: financialSource,
      confidence: 90,
      selected: true,
      needsReview: false,
    },
    {
      id: 'ext-fin-2',
      recordType: 'Property statement',
      description: 'Operating income and expenses summary',
      amount: '$42,180 YTD',
      period: 'Jan – Jun 2025',
      sourceDocumentName: financialSource,
      confidence: 88,
    ...
    {
      id: 'ext-fin-3',
      recordType: 'Expense report',
      description: 'Maintenance and vendor spend',
      amount: '$6,240',
      period: 'Q2 2025',
      sourceDocumentName: financialSource,
      confidence: 86,
  ...
  const imageLabels: ExtractedReviewItem[] = documents.flatMap((doc) =>
    doc.imageLabels.map((tag, index) => ({
      id: `${doc.id}-tag-${index}`,
      uploadedDocumentId: doc.id,
      sourceDocumentName: doc.fileName,
      dataType: 'image_label',
      label: tag,
      value: `Detected in ${doc.fileName}`,
      confidence: 78 + (index % 15),
  ...
  const needsReview: ExtractedReviewItem[] = [
    {
      id: 'ext-review-1',
      uploadedDocumentId: documents[0]?.id ?? 'doc',
      sourceDocumentName: primarySource,
      dataType: 'handwriting',
      label: 'Handwritten move-in note',
      value: 'Tenant requested early key pickup — confirm with Saad Iqbal',
      confidence: 71,
    ...
    },
    {
      ...
      label: 'Crawl space moisture',
      value: 'Standing water in crawl space — verify source before import',
      confidence: 74,
      includeInImport: true,
      needsReview: true,
    },
