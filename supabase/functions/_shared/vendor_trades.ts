/**
 * Keep in sync with: `src/lib/vendorTrades.ts`
 * Deno edge mirror — do not import from `src/`.
 *
 ...
 * - Free-text / description classification uses maintenance_classification rules
 */
import { inferTradeFromText } from "./maintenance_classification/deterministicRules.ts"
...
export function normalizeVendorTrade(
  ...
  // Substring heuristics (issue text / legacy labels) — keep in sync with
  // maintenance_classification/deterministicRules.ts
  ...
  // Phrase-level deterministic rules (handles "leaky faucet", multi-word text)
