...
export function inferIssueTypeFromText(text: string): IssueType | null {
  ...
  if (/\b(leak|leaking|drip|flood(?:ed|ing)?|water damage)\b/.test(d)) return "leak"
  if (/\b(plumb|pipe|drain|toilet|faucet|sink|clog)\b/.test(d)) return "plumbing"
  ...
  if (/\b(hvac|heat|heating|cool|furnace|thermostat|no heat)\b/.test(d)) return "HVAC"
  if (/\b(pest|roach|mouse|rat|bug|insect|termite)\b/.test(d)) return "pest"
  if (/\b(lock|key|deadbolt|door stuck)\b/.test(d)) return "lock"
  return null
}
...
export function issueTypeToCategory(issueType: IssueType | string | undefined): string {
  ...
  if (t === "appliance") return "appliance"
  return "other"
}
...
export function intakeQuestionForStep(
  ...
  step: Exclude<IntakeStep, "awaiting_confirm" | "awaiting_edit_selection" | "submitted">,
): string {
  switch (step) {
    case "issue_type":
    ...
    case "preferred_contact_method":
      return "How would you like us to keep you updated — text message or email?"
    default:
...
export function urgencyQuestion(state: SmsIntakeState, recommended?: string): string {
  ...
  if (level === "emergency" || level === "urgent") {
    ...
    return `Because ${reason}, I'd recommend treating this as ${priorityWord}. Does that sound right, or would you rate it differently? (emergency / urgent / normal / low)`
  }
...
export const EDIT_FIELD_OPTIONS =
  "No problem — what would you like to update? You can say issue type, room/area, urgency, safety, when you noticed, contact method, or description."
...
export const INTAKE_VALIDATION = {
  ...
  contact_method:
    "Would you prefer text message or email for updates?",
} as const
