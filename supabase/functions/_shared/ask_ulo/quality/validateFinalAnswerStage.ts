...
export async function validateFinalAnswer(input: {
  ...
  const needsPeriodSummary = d.needsPeriodSummary ?? (evidence as any).needsPeriodSummary
  const needsOldestWaiting = d.needsOldestWaiting ?? (evidence as any).needsOldestWaiting
  const needsEntityInvestigation = d.needsEntityInvestigation ?? (evidence as any).needsEntityInvestigation
  const deepOpsCandidate = d.deepOpsCandidate ?? (evidence as any).deepOpsCandidate
  const needsDeepOps = d.needsDeepOps ?? (evidence as any).needsDeepOps
  const needsDraftCommunication = d.needsDraftCommunication ?? (evidence as any).needsDraftCommunication
  const needsActiveWorkflows = d.needsActiveWorkflows ?? (evidence as any).needsActiveWorkflows
  const needsWeatherAlerts = d.needsWeatherAlerts ?? (evidence as any).needsWeatherAlerts
  const needsLandlordIncentives = d.needsLandlordIncentives ?? (evidence as any).needsLandlordIncentives
  const needsListResidents = d.needsListResidents ?? (evidence as any).needsListResidents
  const needsPropertyInsights = d.needsPropertyInsights ?? (evidence as any).needsPropertyInsights
  const needsRecurringRepairs = d.needsRecurringRepairs ?? (evidence as any).needsRecurringRepairs
  const needsApproveRepairs = d.needsApproveRepairs ?? (evidence as any).needsApproveRepairs
  const needsMissingUpdates = d.needsMissingUpdates ?? (evidence as any).needsMissingUpdates
  const needsVendorResponseSpeed = d.needsVendorResponseSpeed ?? (evidence as any).needsVendorResponseSpeed
  const needsVendorCompletion = d.needsVendorCompletion ?? (evidence as any).needsVendorCompletion
  const needsVendorInactive = d.needsVendorInactive ?? (evidence as any).needsVendorInactive
  const needsVendorOverload = d.needsVendorOverload ?? (evidence as any).needsVendorOverload
  const needsVendorVerification = d.needsVendorVerification ?? (evidence as any).needsVendorVerification
  const needsVendorBest = d.needsVendorBest ?? (evidence as any).needsVendorBest
  const needsUnitRanking = d.needsUnitRanking ?? (evidence as any).needsUnitRanking
  const needsBriefing = d.needsBriefing ?? (evidence as any).needsBriefing
  const needsRanking = d.needsRanking ?? (evidence as any).needsRanking
