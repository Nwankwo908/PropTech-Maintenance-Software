...
export type AnswerJurisdictionCheck = {
  pass: boolean
  failClosed: boolean
...
const STATE_MENTION_RE: Array<{ code: string; re: RegExp }> = [
  { code: "OR", re: /\b(?:Oregon|\bOR\b)\b/i },
  { code: "WA", re: /\b(?:Washington\s+state|\bWA\b)\b/i },
  { code: "CA", re: /\b(?:California|\bCA\b)\b/i },
  { code: "TX", re: /\b(?:Texas|\bTX\b)\b/i },
  { code: "NY", re: /\b(?:New\s+York|\bNY\b)\b/i },
  { code: "FL", re: /\b(?:Florida|\bFL\b)\b/i },
  { code: "NJ", re: /\b(?:New\s+Jersey|\bNJ\b)\b/i },
  { code: "GA", re: /\b(?:Georgia|\bGA\b)\b/i },
  { code: "AZ", re: /\b(?:Arizona|\bAZ\b)\b/i },
  { code: "NV", re: /\b(?:Nevada|\bNV\b)\b/i },
  { code: "ID", re: /\b(?:Idaho|\bID\b)\b/i },
  { code: "CO", re: /\b(?:Colorado|\bCO\b)\b/i },
]
...
/**
 * Post-answer jurisdiction / scope: correct property, no landlord mix, legal place match.
 */
export function checkAnswerJurisdiction(input: {
  intent: string
  answer: string
  stateCode?: string | null
  cityLabel?: string | null
...
}): AnswerJurisdictionCheck {
  const reasons: string[] = []
  const answer = input.answer.trim()
  const portfolio = (input.portfolioBuildings ?? [])
    .map((b) => b.trim())
    .filter(Boolean)

  // Landlord isolation — never leak foreign IDs / internal landlord keys
  if (/\blandlord_id\b|\borganization_id\b/i.test(answer)) {
    reasons.push("leaked_landlord_identifier")
  }
  const uuids = answer.match(UUID_RE) ?? []
  if (input.landlordId) {
    for (const id of uuids) {
      if (id.toLowerCase() !== input.landlordId.toLowerCase()) {
        reasons.push("foreign_uuid_in_answer")
        break
      }
    }
  } else if (uuids.length > 0) {
    reasons.push("raw_uuid_in_answer")
  }

  // Legal jurisdiction mismatch
  if (input.intent === "legal" && input.stateCode) {
    for (const s of STATE_MENTION_RE) {
      if (s.code === input.stateCode.toUpperCase()) continue
      if (s.re.test(answer)) {
        // "Washington County" in OR is OK when state is OR
        if (
          s.code === "WA" &&
          input.stateCode.toUpperCase() === "OR" &&
          /\bwashington\s+county\b/i.test(answer) &&
          !/\bWashington\s+state\b|\blaws?\s+in\s+Washington\b/i.test(answer)
        ) {
          continue
        }
        reasons.push(`wrong_state_mentioned:${s.code}`)
        break
      }
    }
  }

  // Correct property — when scoped to a building, don’t lead with a different portfolio building
  const scoped = input.buildingFilter?.trim() || null
  if (scoped && portfolio.length > 0) {
    const scopedLower = scoped.toLowerCase()
    const otherBuildings = portfolio.filter(
      (b) => b.toLowerCase() !== scopedLower && !scopedLower.includes(b.toLowerCase()),
    )
    const mentionsScoped =
      answer.toLowerCase().includes(scopedLower) ||
      scopedLower.split(/\s+/).every((w) => w.length > 2 && answer.toLowerCase().includes(w))
    const mentionedOthers = otherBuildings.filter((b) =>
      new RegExp(`\\b${escapeRegExp(b)}\\b`, "i").test(answer)
    )
    if (mentionedOthers.length > 0 && !mentionsScoped) {
      reasons.push(`wrong_property:${mentionedOthers[0]}`)
    }
  }
  ...
  if (needsClarify) reasons.push("legal_without_jurisdiction")

  const pass = reasons.length === 0

  return {
    pass,
    failClosed: failClosed || needsClarify,
    block: needsClarify ? "clarify" : failClosed ? "refuse" : null,
    summary: failClosed || needsClarify
      ? needsClarify
        ? "Legal answer lacks a confirmed jurisdiction."
        : "Answer crossed property, landlord, or jurisdiction scope."
      : "Property and jurisdiction scope look consistent.",
    reasons,
  }
}
