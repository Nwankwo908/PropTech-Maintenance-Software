/**
 * Completeness — did it answer the actual question? Did it overstate uncertainty?
 * Also owns Response Sufficiency Score + Evidence Threshold + Generic Response Filter.
 */

import { looksLikeGenericKpiFallback } from "../tools/maintenance/taskCompletion.ts"
import { hasSubjectMismatch } from "../routing/detectSubject.ts"
import { requiresInvestigation } from "../tools/_shared/investigationDefinition.ts"
import { isStrategicBriefingQuestion } from "../tools/_shared/reasoningFirst.ts"
import {
  isComparisonRankingQuestion,
  isDiagnosisQuestion,
  isRecommendationQuestion,
} from "../routing/reasoningMode.ts"
import { looksLikeIgnoringTier1Intelligence } from "../retrieval/knowledgeHierarchy.ts"
...
export function answerAppearsToAddressQuestion(question: string, answer: string): boolean {
  ...
  if (/^\s*which\b|becoming\s+(?:an?\s+)?emergenc|needs?\s+(?:my\s+)?attention/i.test(q)) {
    ...
    // Named resident / vendor (e.g. "Jordan Lee … late on rent")
    if (
      /\b[A-Z][a-z]+\s+[A-Z][a-z]+\b/.test(a) &&
      /\b(late|overdue|balance|tenant|resident|vendor|electrician|plumber)\b/i.test(
        `${q} ${a}`,
      )
    ) {
      return true
    }
    if (/\b(none|no\s+(?:requests?|tickets?|work\s*orders?|tenants?|residents?)\s+(?:are|look)|did not find|couldn't find)\b/i.test(a)) {
      return true
    }
    return false
  }
...
export type AnswerCompletenessCheck = {
  pass: boolean
  failClosed: boolean
  block: "clarify" | "refuse" | null
  summary: string
  reasons: string[]
  sufficiency: ResponseSufficiencyReport
}

/**
 * Post-answer completeness: answered the ask, didn’t overstate uncertainty when evidence exists.
 */
export function checkAnswerCompleteness(input: {
  question: string
  answer: string
  hasEvidence?: boolean
  packetSatisfied?: boolean
}): AnswerCompletenessCheck {
  const sufficiency = evaluateResponseSufficiency({
  ...
  if (!sufficiency.answersQuestion) {
    reasons.push("did_not_answer_question")
  }
  if (sufficiency.genericFilterFailed) {
    reasons.push("generic_shell_response")
  }
  if (
    input.hasEvidence &&
    looksLikeIgnoringTier1Intelligence(input.answer)
  ) {
    reasons.push("overstated_uncertainty_with_evidence")
  }

  const failClosed =
    reasons.includes("overstated_uncertainty_with_evidence") ||
    (sufficiency.status === "fail" &&
      (reasons.includes("did_not_answer_question") ||
        reasons.includes("generic_shell_response")) &&
      !input.packetSatisfied)

  const pass = reasons.length === 0 && sufficiency.status !== "fail"
  ...
  return {
    ...
    summary: failClosed
      ? reasons.includes("overstated_uncertainty_with_evidence")
        ? "Answer claims insufficient information while evidence was available."
        : "Answer does not complete the user’s question."
      : pass
        ? "Answer addresses the question with appropriate confidence."
        : sufficiency.summary,
    reasons,
    sufficiency,
  }
