...
export async function vendorVerificationStatusLookup(
  ...
  const [
    ...
    supabase
      ...
      .select(
        "id, vendor_id, status, business_name, contact_name, license_status, license_number, license_state, coi_general_liability, coi_expiration, coi_additional_insured, coi_status, background_check_status, w9_received, tax_entity_type, tin_type, tin_last4, tin_fingerprint, w9_variant, tax_1099_treatment, trade_categories, service_area, availability",
      )
      .eq("landlord_id", landlordId),
  ])
