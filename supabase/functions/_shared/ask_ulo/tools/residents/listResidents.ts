...
export type ListResidentsParams = {
  ...
  unitId?: string | null
  /** Building name filter (e.g. Maple Heights) when propertyId is unresolved. */
...
async function listLateRent(
  ...
  const result = await searchLateRent(supabase, {
    organizationId,
  ...
  if (!result.success || !result.data) {
    return emptyResult(baseParams, result.error ?? "Late rent lookup failed")
  }

  const residents: ResidentEvidence[] = result.data.rows.map((r) => ({
    residentId: r.residentId,
    name: r.name,
    unitLabel: r.unitLabel,
    propertyName: r.propertyName,
  ...
  const citations: AskUloCitation[] = result.evidence.slice(0, 5).map((e) => ({
    tool: "structured",
    title: e.label,
    citation: e.source,
    excerpt: e.excerpt ?? "",
