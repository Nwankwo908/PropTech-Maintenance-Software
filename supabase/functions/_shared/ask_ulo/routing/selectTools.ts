...
import {
  ...
  getDomainTool,
  type DomainToolId,
} from "../tools/_shared/registry.ts"
...
const TOOL_PARAMETER_SCHEMAS: Partial<Record<DomainToolId, Record<string, unknown>>> = {
  ...
  rank_properties: {
    ...
    properties: {
      limit: {
        ...
        minimum: 1,
        maximum: 50,
        description: "Max properties to return in the ranking.",
      },
    },
    additionalProperties: false,
  },
  search_operations_graph: {
    type: "object",
    properties: {
      query: { type: "string", description: "Optional free-text focus for ops activity." },
      eventLimit: {
        type: "integer",
        minimum: 1,
        maximum: 50,
        description: "Max recent graph events to scan.",
      },
    },
    additionalProperties: false,
  },
  search_legal_sources: {
    type: "object",
    properties: {
      query: { type: "string", description: "Legal question to retrieve sources for." },
      stateCode: { type: "string", description: "US state code (e.g. OR, NJ)." },
      citySlug: { type: "string" },
      countySlug: { type: "string" },
      housingProgram: { type: "string", description: "Optional housing program filter (e.g. Section 8)." },
      includeRag: { type: "boolean" },
      includeStructured: { type: "boolean" },
      matchCount: { type: "integer", minimum: 1, maximum: 20 },
    },
    additionalProperties: false,
  },
