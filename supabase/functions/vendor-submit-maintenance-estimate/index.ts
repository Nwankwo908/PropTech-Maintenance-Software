...
import { normalizeVendorJobToken } from "../_shared/vendorJobActionToken.ts"
...
serve(async (req) => {
  ...
  const token = normalizeVendorJobToken(
  ...
  try {
    ...
    console.error("[vendor-submit-maintenance-estimate]", err)
    return jsonResponse({ error: "Could not complete this estimate request." }, 500)
  }
})
