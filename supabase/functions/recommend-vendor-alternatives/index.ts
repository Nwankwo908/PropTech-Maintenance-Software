...
serve(async (req) => {
  ...
  if (!Deno.env.get("ADMIN_REASSIGN_SECRET")?.trim()) {
    console.error("[recommend-vendor-alternatives] ADMIN_REASSIGN_SECRET not set")
    return jsonResponse({ error: "Server misconfiguration" }, 500)
  }

  if (!adminReassignSecretAuthorized(req)) {
    console.warn(
      "[recommend-vendor-alternatives] 401 Unauthorized: x-admin-reassign-secret (or legacy Bearer admin secret) does not match ADMIN_REASSIGN_SECRET on this deployment",
    )
