...
3. SMS the landlord before any assignment. List every Active matchable vendor in the **same state** as the property (specialists first, then handymen). One vendor → reply YES. More than one → reply 1 / 2 / 3…. Do not offer a vendor whose service area is a different state.
...
Configure in **Twilio Console → Phone Numbers → your DID** (or Messaging Service):

| Webhook | URL |
|---------|-----|
| **Inbound** | `https://YOUR_REF.supabase.co/functions/v1/sms-inbound` |
| **Status callback** | `https://YOUR_REF.supabase.co/functions/v1/sms-status-callback` |

Alpha (`mzpqwuizhiaczxcnmxbt`) — inbound and status URLs:

- Inbound: `https://mzpqwuizhiaczxcnmxbt.supabase.co/functions/v1/sms-inbound`
- Status: `https://mzpqwuizhiaczxcnmxbt.supabase.co/functions/v1/sms-status-callback` (`TWILIO_STATUS_CALLBACK_URL`)
- From line: `+18775803356` (`TWILIO_FROM_NUMBER` / Limited Alpha shared DID)
