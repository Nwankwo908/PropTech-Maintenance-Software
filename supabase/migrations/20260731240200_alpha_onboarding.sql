-- Alpha runs the same onboarding wizard as New Landlord until setup completes.

-- Allow staff dashboard reset to purge Alpha portfolio (service role keeps access).
...
grant execute on function public.purge_landlord_portfolio(uuid) to authenticated;

-- Fresh Alpha account: start on the onboarding welcome hub.
insert into public.landlord_onboarding (
  landlord_id,
  onboarding_status,
  current_step,
  properties,
...
on conflict (landlord_id) do update set
  ...
  completed_at = null,
  updated_at = now()
where public.landlord_onboarding.landlord_id = '068daf53-07e4-4493-bd7f-6106e3c8c62f';
